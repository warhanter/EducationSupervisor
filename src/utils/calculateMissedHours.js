export function calculateMissedHours(
  ...
  const sickLeaveStrings = new Set();

  sickLeave.forEach((sickDay) => {
    if (sickDay.start && sickDay.end) {
      // نطاق عطلة
      const start = new Date(sickDay.start);
      start.setHours(0, 0, 0, 0);
      const end = new Date(sickDay.end);
      end.setHours(0, 0, 0, 0);

      const current = new Date(start);
      while (current <= end) {
        sickDayStrings.add(current.toISOString().split("T")[0]);
        current.setDate(current.getDate() + 1);
      }
    } else {
      // يوم عطلة منفرد
      holidayStrings.add(new Date(sickLeave).toISOString().split("T")[0]);
    }
  });
  ...
  while (current <= endDateOnly) {
    ...
    const isSick = sickLeaveStrings.has(currentDateString);
    ...
    detailedDays.push({
      ...
      dayName: dayName,
      hours: !isSick ? parseFloat(Math.ceil(hoursForDay)) : 0,
      justifiedHours: isSick ? parseFloat(Math.ceil(hoursForDay)) : 0,
